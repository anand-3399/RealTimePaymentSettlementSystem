...
The **Payment Processor** is the central orchestration service in the RTPS system. It consumes order events from Kafka, executes bank transfers via Bank1, and guarantees that no payment is ever silently lost — even in the face of infrastructure failures.
...
│  │  - Calls Bank1Client.transferMoney(...)                     │   │
...
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │ Bank1Client (Resilience4j)                                   │   │
│  │  - @Retry: 3 attempts, exponential backoff (2s → 4s → 8s)   │   │
│  │  - @CircuitBreaker: opens after failures, 60s cool-down      │   │
│  │  - Fallback: returns PENDING_RETRY on Bank1 unavailability  │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │ TransferService                                               │   │
...
| Bank1 unavailable during transfer | Resilience4j retries 3× with exponential backoff | Fallback sets status to `PENDING_RETRY`; scheduler recovers |
| Bank1 down for extended period | Circuit Breaker opens; fast-fail prevents resource waste | Circuit Breaker re-probes after 60s cool-down period |
...
| Payment Processor → Bank1 | `X-Internal-Secret` | `rtps-internal-secret-2026-PPTAJ` |

Secrets are injected via environment variables:
- `ORDER_TO_PROCESSOR_SECRET`
- `PROCESSOR_TO_BANK1_SECRET`
...
| `PaymentService` | Core business logic: idempotency, payment persistence, Bank1 orchestration |
| `Bank1Client` | HTTP client to Bank1 with Resilience4j `@Retry` and `@CircuitBreaker` |
