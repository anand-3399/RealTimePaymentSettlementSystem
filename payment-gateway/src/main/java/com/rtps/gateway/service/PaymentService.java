...
public class PaymentService {
    ...
    @Autowired
    private Bank1Client ajBankClient;
    ...
    public void processPayment(UUID orderId, String userId, BigDecimal amount, String currency, 
        ...
        if (existingPayment.isPresent()) {
            log.warn("Payment already exists for orderId: {} | currentStatus: {}", 
        ...
        Payment payment = Payment.builder()
                ...
                .retryCount(0)
                .maxRetries(configService.getConfigAsInt("RETRY_MAX_ATTEMPTS"))
                .build();
    ...
    private void executeHandshake(Payment payment) {
        try {
            Bank1Request ajRequest = Bank1Request.builder()
            ...
            if ("LOCKED_PENDING_RETRY".equals(ajResponse.getStatus())) {
                ...
                // The user specifically requested that FAILED status should be picked up by the retry scheduler
                // Using handleInfrastructureFailure delegates it to PENDING_RETRY with exponential backoff
                ...
                log.info("Handshake successful (201 CREATED) for payment {}. Status: SENT_AWAITING_RESPONSE. Waiting for webhook.", payment.getId());
                logTransaction(payment, "BANK1_HANDSHAKE", "PENDING", "SENT_AWAITING_RESPONSE", "Handshake Accepted (201): " + ajResponse.getMessage());
            } else {
    ...
    private void handleLockContention(Payment payment, String message) {
        ...
        // Exponential backoff
        long baseDelay = configService.getConfigAsLong("RETRY_BASE_DELAY_ACCOUNT_LOCKED");
        ...
        log.warn("Account Locked for payment {}. Retrying in {}s | attempt: {}", payment.getId(), delaySeconds, payment.getRetryCount());
    ...
    private void handleInfrastructureFailure(Payment payment, String message) {
        ...
        long initialDelay = configService.getConfigAsLong("RETRY_BASE_DELAY_BANK_UNAVAILABLE"); 
        ...
        log.warn("Bank Unavailable for payment {}. Retrying in {}s | attempt: {}", payment.getId(), delaySeconds, payment.getRetryCount());
        logTransaction(payment, "INFRA_FAILURE", oldStatus, "PENDING_RETRY", message);
    }

    @Transactional
    public void retryPaymentExecution(Payment payment) {
        log.info("Executing retry for payment {} | status: {} | attempt: {}", 
    ...
    @Autowired
    private com.rtps.gateway.util.WebhookSignatureVerifier signatureVerifier;
        ...
        try {
            ...
            paymentOpt.ifPresentOrElse(payment -> {
                ...
                if (payment.getStatus() == Payment.PaymentStatus.SENT_AWAITING_RESPONSE || 
                    ...
                    logTransactionInternal(payment, "BANK1_WEBHOOK_UPDATE", oldStatus, 
                    ...
                    log.info("Payment {} already in final status {}. No update needed.", payment.getId(), payment.getStatus());
                }
            }, () -> {
                webhookLogRepository.save(logEntry);
