...
public class BackgroundSyncScheduler {
	...
	BackgroundSyncScheduler(OperationalBankAccountRepository accountRepository,
			SecureBankAccountRepository secureAccountRepository) {
		this.accountRepository = accountRepository;
		this.secureAccountRepository = secureAccountRepository;
	}

	// Use SpEL to read the delay dynamically from ConfigService
	@Scheduled(fixedDelayString = "#{@configService.getConfigAsString('AJBANK_BACKGROUND_SYNC_DELAY')}")
	public void syncAccountsFromSecureDb() {
		...
		for (OperationalBankAccount localAccount : accounts) {
			SecureBankAccount secureAccount = secureAccountRepository
					.findByAccountNumber(localAccount.getAccountNumber()).orElse(null);

			if (secureAccount != null && localAccount.getAccountVersion() < secureAccount.getAccountVersion()) {
				log.info("Background Sync: Updating account {} from version {} to {}", localAccount.getAccountNumber(),
						localAccount.getAccountVersion(), secureAccount.getAccountVersion());
