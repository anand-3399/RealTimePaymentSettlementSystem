...
public class GlobalExceptionHandler {
	...
	GlobalExceptionHandler(LockTimeoutRepository lockTimeoutRepository) {
		this.lockTimeoutRepository = lockTimeoutRepository;
	}
	...
	public ResponseEntity<TransferResponse> handleLockException(Exception e) {
		...
		LockTimeout alert = LockTimeout.builder().correlationId(correlationId)
		...
		TransferResponse response = TransferResponse.builder().status("PENDING_RETRY").correlationId(correlationId)
				.build();
	...
	public ResponseEntity<TransferResponse> handleBusinessValidationException(BusinessValidationException e) {
		...
		TransferResponse response = TransferResponse.builder().status("FAILED").message(e.getMessage())
	...
	public ResponseEntity<TransferResponse> handleGenericException(Exception e) {
		...
		TransferResponse response = TransferResponse.builder().status("FAILED").message("System cannot process")
				.correlationId(correlationId).build();
